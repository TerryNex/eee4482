...
function DeleteUser($request, $response, $args) {
    ...
    try {
        ...
        $user_id_delete =  $data['user_id_delete']??$args['user_id_delete'] ?? null;
        $email = $data['email'] ?? '';
        ...
        $stmt->bindParam(':delete_user_id', $params[':user_id'], PDO::PARAM_INT);
        $stmt->execute();
        // remove borrow_history
        $stmt = $conn->prepare("DELETE FROM borrowing_history WHERE user_id = :delete_user_id");
        $stmt->bindParam(':delete_user_id', $params[':user_id'], PDO::PARAM_INT);
        $stmt->execute();
