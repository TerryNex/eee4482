...
function AddBook($request, $response, $args) {
    ...
    $title      = $data['title'] ?? '';
    $authors    = $data['authors'] ?? ''; // the font-end returns the author field
    ...
    try {
        ...
        if ($result) {
            $response->getBody()->write(json_encode(["message" => "ISBN already exists"]));
            return $response->withHeader('content-type', 'application/json')->withStatus(400);
        }

        // use prepared statements
        $sql = "INSERT INTO books (title, authors, publishers, date, isbn, status) VALUES (:title, :authors, :publishers, :date, :isbn, 0)";
        $stmt = $conn->prepare($sql);
